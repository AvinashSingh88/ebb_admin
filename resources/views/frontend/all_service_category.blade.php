...
<style>
 ...
 <section class="pageTitle">
    ...
    <li class="link-category link-category1aa">
						...
						@php
						   $countCategory = \App\Models\Category::where('parent_id','=',0)->where('type','2')->count();
						@endphp
                ...
                <div class="link-categoryx link-category1az ">
                   <ul class="list-unstyled dropdown-list">
					 @foreach (\App\Models\Category::where('parent_id', '=', '0')->where('type','2')->get() as $key => $category)
						 ...
						 @endphp
							<li><a class="dropdown-item1" href="{{ route('servicecat', $category->slug) }}">{{  $category->getTranslation('name') }} ({{ $SubCategoryCountByCat}}) </a></li>
                     @endforeach
                   </ul>
                </div>
             </div>
