...
$(document).on('click', '#shopid', function() {
    $(this).toggleClass('active2');
...
$('#ckeckboxselect').on('keyup', '#txtp', function() {
    var qut = $(this).val();


    var price = $(this).parent('td').parent('tr').find("#price").text();
    var discount = $(this).parent('td').parent('tr').find("#discount").text();
    var gst = $(this).parent('td').parent('tr').find("#gst").text();
    var total = $(this).parent('td').parent('tr').find("#txttotle").text();


    var percentage_discount = (price - (price * discount / 100));
    var percentage_gst = (percentage_discount + (percentage_discount * gst / 100));
    var grand_Total = percentage_gst * qut;
...
$('#ckeckboxselect').on('change', '#boxselect', function() {
    ...
    if (this.checked) {
        ...
        if (qut == 0) {
            ...
            var percentage_discount = (price - (price * discount / 100));
            var percentage_gst = (percentage_discount + (percentage_discount * gst / 100));
            var grand_Total = percentage_gst * 1;
            $(this).parent('td').parent('tr').find("#txttotle").val(grand_Total);
        } else {
            $(this).parent('td').parent('tr').children('td').find("#txtp").val(qut);
            var percentage_discount = (price - (price * discount / 100));
            var percentage_gst = (percentage_discount + (percentage_discount * gst / 100));
            var grand_Total = percentage_gst * qut;
            $(this).parent('td').parent('tr').find("#txttotle").val(grand_Total);
        ...
        if (qut == 1) {
            ...
            var percentage_discount = (price - (price * discount / 100));
            var percentage_gst = (percentage_discount + (percentage_discount * gst / 100));
            var grand_Total = percentage_gst * 0;
            $(this).parent('td').parent('tr').find("#txttotle").val(grand_Total);
        } else {
            $(this).parent('td').parent('tr').children('td').find("#txtp").val(qut);
            var percentage_discount = (price - (price * discount / 100));
